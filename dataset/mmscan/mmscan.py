...
from .utils.box_utils import from_9dof_to_6dof
from .utils.data_io import id_mapping, load_json, read_annotation_pickle
from .utils.task_utils import anno_token_flatten
...
class MMScan(Dataset):
    ...
    def __init__(
        ...
        self.mmscan_anno = load_json(f'{self.lang_anno_path}/MMScan_samples/' +
                                     f'{self.task_anno_mapping[task]}')[
                                         self.split]
        if ratio < 1.0:
    ...
    def __getitem__(self, index_: int) -> dict:
        ...
        1. Scan-level data:
            ...
            - "images" (list[dict]): A list of camera information,
                ...
                - 'intrinsic' (np.ndarray):
                    Camera intrinsic matrix of the RGB image.
                - 'depth_intrinsic' (np.ndarray):
                    Camera intrinsic matrix of the depth image.
                - 'extrinsic' (np.ndarray): Camera extrinsic matrix.
                - 'visible_instance_id' (list):
                    IDs of objects visible in the image.
        ...
        2. Anno-level data
                ...
                - "object_ids" (list[int]):
                        Object IDs referenced in the question.
                - "object_names" (list[str]):
                        Types of referenced objects.
                - "input_bboxes_id" (list[int]):
                        IDs of input bounding boxes.
                - "input_bboxes" (list[np.ndarray]):
    ...
    def data_collect(self):
        ...
        samples, they need to be flattened.
        """
    ...
    def __check_lang_anno__(self, sample: dict) -> bool:
        ...
        if self.task == 'MMScan-VG':
            ...
            for object_id in object_ids:
                if (object_id not in anno_obj_ids):
                    return False
        if self.task == 'MMScan-QA':
